import { getLibraryServices } from '@/api/portal/api-library';
...
export default async function LibraryServicePage() {
  ...
  function mapId(tieuDe: string, id: number) {
    return `/service/library-service/${mapSlugWithId(tieuDe, id)}`;
  }
  return (
    <>
      ...
      <section>
        {data.map((item) => (
          <CustomLink key={item.id} href={mapId(item.tieuDe, item.id)} className="mt-3">
